...
export default function Register() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      setMsg("تم ارسال رسالة تحقق الى بريدك الالكتروني ، شيك عليه وارجع لنا");
    } catch (error) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4">
      <form onSubmit={onSubmit} className="w-full max-w-md bg-white rounded-2xl shadow p-6 space-y-4">
        <h1 className="text-2xl font-bold">سو لك حساب</h1>
        ...
        <div>
          ...
          <select className="mt-1 w-full border rounded-xl p-2" value={role} onChange={(e) => setRole(e.target.value)}>
            ...
            <option value="vendor">مالك عربة طعام</option>
          </select>
        </div>
        ...
        <button className="w-full bg-black text-white rounded-xl p-2">تسجيل</button>
      </form>
    </div>
  );
}